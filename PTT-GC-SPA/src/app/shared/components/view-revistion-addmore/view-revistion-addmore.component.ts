...
export class ViewRevistionAddmoreComponent implements OnInit {
  ...
  capexInformationForm: FormGroup = this.fb.group({
  ...
  get invalidSubmit() {
    return this.capexInformationForm.valid;
  }
  ...
  ngOnInit(): void {
    ...
    this.initiativeService.GetOwners().subscribe(owners => this.owners = owners, error => this.unauthorized.error(error));
  ...
  getPool(poolId): Promise<any> {
    return new Promise((resolve, reject) => {
      this.capexService.GetPoolInnitiativeByID(poolId).subscribe(response2 => {
        if (response2 != null && response2 != undefined) {
          this.poolName = response2[0]?.name;
        }
        resolve(true);
      });
    });
  ...
  GetAvailableBudget(poolId): Promise<any> {
    return new Promise((resolve, reject) => {
      this.capexService.GetCapexsInfo(poolId, 'Requestpool').subscribe(response2 => {
        if (response2 != null && response2 != undefined) {
  ...
  GetInitiative() {
    // this.capexService.revistionData
    // this.capexService.GetCapexsInfo(this.id.toString(), "AddmoreCapex").subscribe(async response => {
    this.capexService.GetCapexsInformationsByCapexInformationId(this.Capexid).subscribe(async response => {
      if (response) {
        ...
        this.ProjectCost = parseFloat(cost_).toFixed(2);
        this.ExistingCost = parseFloat((cost_ - AdditionalCost_).toString()).toFixed(2);
        this.AdditionalCost = parseFloat(AdditionalCost_.toString()).toFixed(2);

        this.SpendingActual = parseFloat(response.spendingActual).toFixed(2);
        ...
        this.BudgetAvailable = parseFloat((+this.ExistingCost - this.SpendingActual).toString()).toFixed(2);

        this.TotalAvailable = parseFloat((+this.BudgetAvailable + +this.AdditionalCost).toString()).toFixed(2);
        ...
        if (response.poolId && response.poolId != 0) {
          await this.GetAvailableBudget(response.poolId);
          await this.getPool(response.poolId);
        }
        this.capexInformationForm.patchValue({
          ...
          ExistingCost: parseFloat(this.ExistingCost).toFixed(2),
          ...
          ShowRemark_tmp: false,
          SpendingActual: parseFloat(response.spendingActual).toFixed(2),

          BudgetAvailable: parseFloat(this.BudgetAvailable).toFixed(2),
          TotalAvailable: parseFloat(this.TotalAvailable).toFixed(2),
          PoolYear: response.budgetYear,
          PoolID: this.poolName, // response.poolId + '-->' + this.getPool(response.poolId),
        ...
        this.swalTool.Error('Not Have Data');
  ...
  setAdditionalCost(value) {
    ...
    // this.capexInformationForm.controls['ProjectCost'].setValue( this.ProjectCost.replace(/,/g, ""));
    ...
    if (this.RequestIniNoDate != date1) {
      ...
      if (this.RequestIniNoDate != "") {
        ...
        if ((d3 > d1 && d3 < d2) == false) {
          ...
          this.capexInformationForm.controls['RequestIniNoDate'].setValue('');
          // this.capexInformationForm.patchValue({ RequestIniNoDate: this.RequestIniNoDate });
  ...
  SetMarkAsTouchedFormGeneral() {
    ...
    this.capexInformationForm.controls.StartingDate.markAsTouched();
  ...
  bindMonth() {
    for (let i = 0; i < +this.year_m.length; i++) {
      this.capexService.GetMonthlyInvestmentPlan(this.id.toString(), this.Capexid,
        this.year_m[i]).subscribe(resp => {
  ...
  CHk_validate() {
    ...
    try {
      ...
      if (+AnnualTotal_overall.value != this.capexInformationForm.controls['AdditionalCost'].value) {
        this.tmp_error = "yes";
      } else {
        this.tmp_error = "no";
      }
