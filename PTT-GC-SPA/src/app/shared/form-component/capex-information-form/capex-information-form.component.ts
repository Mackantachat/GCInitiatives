import { Component, OnInit, ChangeDetectorRef, Input, AfterViewInit, ViewChild, QueryList, OnDestroy, OnChanges, ChangeDetectionStrategy } from '@angular/core';
...
@Component({
    selector: 'app-capex-information-form',
    templateUrl: './capex-information-form.component.html',
    styleUrls: ['./capex-information-form.component.css'],
})
export class CapexInformationFormComponent implements OnInit, OnDestroy, OnChanges {
    constructor(
    ...
    @Input() id: number;
    @Input() page: string;
    @Input() generalData: Initiative = {} as Initiative;
    generalDatas: Initiative = {} as Initiative;

    @Input() costEstFromGeneral: number;
    ...
    capexTabStatusIsDisable: boolean = false;
    ...
    getFormError(field) {
        return (this.formGroup.get('capexInformationForm').get(field).touched || this.formGroup.get('capexInformationForm').get(field).dirty) && this.formGroup.get('capexInformationForm').get(field).invalid;
    }
    ...
    get annualForm_() {
        return this.formGroup.get('capexInformationForm').get('AnnualForm').get('annualForm_list') as FormArray;
    }
    ...
    ngOnChanges() {
        ...
        this.compareCostEstAndProjCost();
    }
    ...
    ClearOwnerName() {
        //clear
        this.formGroup.get('capexInformationForm').get('codeCostCenterOfVP').setValue(null);
                ...
                if (response.length != 0) {
                    ...
                    if (!this.generalData.requestProjectEngineer) {
                        this.progressService.getResponsibleNoEng(response[0].mainPositionCostCenter).then((getRespoRes) => {
                            ...
                            // this.formGroup.get('progressForm')?.get('responsible')?.patchValue(getRespoRes[0].attribute01);
                        });
                    }
    ...
    if (this.page == 'pool-create') {
      ...
      this.initiativeService.DisableTabCapex = false;
    }
    //for pool
    if (this.id == null && this.page == 'pool-create') {
      this.initiativeService.DisableTabCapex = false;
            ...
            //   this.year_next = (new Date()).getFullYear() + 1;
            // }else{
            //   this.year_next = (new Date()).getFullYear();
            // }
            ...
            this.capexInformationForm.patchValue({
                ...
                budgetAvailable: 0,
                totalAvailable: 0,
                actionYear: this.dateUtil.GetToday,
            ...
            // console.log(`..DEB: before compare childMessage = ${this.costEstFromGeneral}`);
            // To compare CostEst and Project Cost.
            this.costEstFromGeneral = +this.generalData.costEstCapex;
            this.compareCostEstAndProjCost();
            ...
            this.initiativeService.GetSuggestStatus(this.initiativeService.id).subscribe(response => {
                ...
                if (response) {
                    ...
                    this.is_RequestCapex = response.isRequestCapex ? true : false;
                    this.is_Max = response.max ? true : false;
            ...
            this.initiativeService.GetInitiative(this.initiativeService.id).subscribe((initiativeResponse) => {
                if (initiativeResponse) {
                    ...
                    this.capexService.GetCapexInformationList(this.id.toString()).subscribe(response => {
                        ...
                        if (response.length == 0) {
                            ...
                            let cost_;
                            //get data form General form
                            let costEstCapexType = initiativeResponse.costEstCapexType;
                            let costEstCapex = initiativeResponse.costEstCapex ? (initiativeResponse.costEstCapex).toString() : "0";
                            let fxExchange = initiativeResponse.fxExchange ? (initiativeResponse.fxExchange).toString() : "0";
                            let startingDate = initiativeResponse.startingDate;
                            let finishingDate = initiativeResponse.finishingDate;
                            if (costEstCapexType == "USD") {
                                cost_ = parseFloat(costEstCapex) * parseFloat(fxExchange);
                            }
                            else {
                                cost_ = parseFloat(costEstCapex);
                            }
                            this.StartingDate = startingDate ? new Date(startingDate) : null;
                            this.RequestIniNoDate = startingDate ? new Date(startingDate) : null;
                            this.ProjecctComRun = finishingDate ? new Date(finishingDate) : null;
                            this.ProjectExePeriodYear = "0";
                            this.ProjectExePeriodMonth = "0";
                            this.BudgetStartDate = "";
            ...
            } else if (response.length > 0 && this.initiativeService.isAddmore) {
              this.formGroup.get("initiativesForm").get("isRequestCapex").patchValue(true);
              let gate2Data: PimGate = this.capexService.gate2Data.value;
              this.capexDisableField = false;
              this.initiativeService.DisableTabCapex = false;
              //add more new reccord
              this.year_now = (new Date()).getFullYear();
              this.year_next = (new Date()).getFullYear() + 1;
              // if(new Date().getMonth() >= 8){
              //   this.year_next = (new Date()).getFullYear() + 1;
              ...
              //change get form value and this.initativeService.suggestion
              //get data form General form
              let startingDate = response[response.length - 1].startingDate;
              let finishingDate = response[response.length - 1].projecctComRun;
              let requestIniNoDate = response[response.length - 1].requestIniNoDate;
              this.StartingDate = startingDate ? new Date(startingDate) : null;
              this.ProjecctComRun = finishingDate ? new Date(finishingDate) : null;
              this.RequestIniNoDate = requestIniNoDate ? new Date(requestIniNoDate) : null;
              this.ProjectExePeriodYear = response[response.length - 1].projectExePeriodYear;
              this.ProjectExePeriodMonth = response[response.length - 1].projectExePeriodMonth;
              this.BudgetStartDate = "";
              this.ProjectCost = response[response.length - 1].projectCost.toFixed(2);
              this.OldProjectCost = response[response.length - 1].projectCost.toFixed(2);
              this.organization = "";
              this.ReasonOfChanging = "";
              this.BudgetForm = "";
              this.BetweenYear = "";
              ...
              this.PoolBudgetForm = "";
              this.strategy = "MillionBaht";
              this.CostCenterOfVP = response[response.length - 1].costCenterOfVP;
              this.CodeCostCenterOfVP = response[response.length - 1].codeCostCenterOfVP;
              this.PoolYear = this.year_now;
                            ...
                            this.capexInformationForm.patchValue({
                                ...
                                existingCost: this.ProjectCost,
                                existingBudget: this.ProjectCost,
                                budgetAvailable: this.ProjectCost - response[response.length - 1].spendingActual,
                                //additionalCost: gate2Data.additionalOpexBudget,  //default value from gate 2
                                additionalCost: null,
                                totalAvailable: this.ProjectCost - response[response.length - 1].spendingActual,
                                // startingDate_tmp: response.startingDate,
                                // projecctComRun_tmp: response.finishingDate,
                                requestIniNoDate_tmp: null,
                                showRemark_tmp: false,
                                ...
                                //Sequent
                                sequent: response[response.length - 1].sequent ? response[response.length - 1].sequent + 1 : 1,
                                revistion: response[response.length - 1].revistion ? response[response.length - 1].revistion + 1 : 1,
                                capexType: "AddmoreCapex",
                                actionYear: this.dateUtil.GetToday,
                                spendingActual: response[response.length - 1].spendingActual,
                                //Revistion
                                //CapexType

                                //Temporary First Addmore
                                firstAddmoreStatus: "addmore"
                            });
                            // To compare CostEst and Project Cost.
                            this.costEstFromGeneral = +this.generalData.costEstCapex;
                            this.compareCostEstAndProjCost();
                            //let poolID_ = response[response.length - 1].poolId;

                            // if (poolID_ != 0) {

                            //   this.capexService.GetCapexsInfo(poolID_, 'Requestpool').subscribe(response2 => {
                            //     if (response2 != null && response2 != undefined) {
                            //       // if (response[response.length - 1].availableBudget == 0) {
                            //       this.AvailableBudget = response2.availableBudget;
                            //       if (this.initiativeService.suggestionStatus.status === 'revised') {
                            //         this.AvailableBudget += parseFloat(this.capexInformationForm.get('additionalCost').value);
                            //       }
                            //       // }
                            //       // else {
                            //       //   this.AvailableBudget = response[response.length - 1].availableBudget;
                            //       // }
              ...
              //set flag capex loaded
              this.progressService.capexLoading = true;
            } else if (response.length > 0 && this.initiativeService.isReturn) {
              this.formGroup.get("initiativesForm").get("isRequestCapex").patchValue(true);
              this.capexDisableField = false;
              this.initiativeService.DisableTabCapex = false;
              //add more new reccord
              ...
              //change get form value and this.initativeService.suggestion
                            ...
                            this.initiativeService.DisableTabCapex = (initiativeResponse.capexTabStatus && initiativeResponse.capexTabStatus == 2) || initiativeResponse.isReviseFlow ? true : false;
                            //get data
                            // return cost
                            this.returnCost = response[response.length - 1].returnCost;

                            this.capexInformationForm.patchValue(response[response.length - 1]);
                            this.year_now = (new Date()).getFullYear();
                            this.year_next = (new Date()).getFullYear() + 1;
                            ...
                            this.seq_ = response[response.length - 1].sequent;

                            if (response[response.length - 1].capexType == 'AddmoreCapex') {
                                this.initiativeService.isAddmore = true;
                            } else if (response[response.length - 1].capexType == 'ReturnCapex') {
                                this.initiativeService.isReturn = true;
                            } else if (response[response.length - 1].capexType == 'ReviseCapex') {
                                this.initiativeService.isRevise = true;
                            }

                            //this.capexInformationForm.get('existingCost').patchValue(response[response.length - 1].projectCost);

                            this.capexInformationId = response[response.length - 1].capexInformationId;

                            this.StartingDate = response[response.length - 1].startingDate ? new Date(response[response.length - 1].startingDate) : new Date(initiativeResponse.startingDate);

                            this.ProjecctComRun = response[response.length - 1].projecctComRun ? new Date(response[response.length - 1].projecctComRun) : new Date(initiativeResponse.finishingDate);
                            this.capexInformationForm.patchValue({
                                startingDate: this.StartingDate,
                                projecctComRun: this.ProjecctComRun
                            });
                            this.ProjectExePeriodYear = response[response.length - 1].projectExePeriodYear;
                            this.ProjectExePeriodMonth = response[response.length - 1].projectExePeriodMonth;
                            if (response[response.length - 1].requestIniNoDate != null) {
                                this.RequestIniNoDate = response[response.length - 1].requestIniNoDate ? new Date(response[response.length - 1].requestIniNoDate) : null;
                                this.BudgetStartDate = response[response.length - 1].requestIniNoDate ? new Date(response[response.length - 1].requestIniNoDate) : null;
                            }
                            else {
                            ...
                            this.ProjectCost = response[response.length - 1].projectCost ? (response[response.length - 1].projectCost).toFixed(2) : 0.00;
                            this.OldProjectCost = response[response.length - 1].projectCost ? (response[response.length - 1].projectCost).toFixed(2) : 0.00;
                            this.organization = "";
                            this.ReasonOfChanging = response[response.length - 1].reasonOfChanging;

                            if (response[response.length - 1].reasonOfChanging != "" && response[response.length - 1].reasonOfChanging != null) {
                                this.RemarkShow = true;
                            }

                            // if (new Date(response[response.length - 1].actionYear).getMonth() >= 8) {
                            //   this.year_now = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear();
                            //   this.year_next = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear + 1 : (new Date()).getFullYear() + 1;
                            // } else {
                            //   this.year_now = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear();
                            //   this.year_next = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear();
                            // }


                            if (response[response.length - 1].budgetPeriod == "Annual") {
                                this.BudgetForm = "Annual";
                                this.year_now = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear - 1 : (new Date()).getFullYear();
                                this.year_next = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear() + 1;
                            }
                            else if (response[response.length - 1].budgetPeriod == "Mid Year") {
                                this.BudgetForm = "Mid Year";
                                this.year_now = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear();
                                this.year_next = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear + 1 : (new Date()).getFullYear() + 1;
                            }
                            else if (response[response.length - 1].budgetPeriod == "Current year") {
                                this.year_now = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear : (new Date()).getFullYear();
                                this.year_next = response[response.length - 1].budgetYear ? +response[response.length - 1].budgetYear + 1 : (new Date()).getFullYear() + 1;
                            ...
                            if (response[response.length - 1].betweenYear == "Contingency") {
                                this.BetweenYear = "Contingency";
                            }
                            else if (response[response.length - 1].betweenYear == "Transfer") {
                                this.BetweenYear = "Transfer";
                                this.inshow = true;
                                this.ferPool = false;
                            }
                            // else if (response[response.length - 1].betweenYear == "BOD Approval on") {
                            //   this.BetweenYear = "BOD Approval on";
                            // }
                            else if (response[response.length - 1].betweenYear == "BOD Approval") {
                                this.BetweenYear = "BOD Approval";
                                this.inshow = false;
                                this.ferPool = false;
                            }
                            else if (response[response.length - 1].betweenYear == "Pool Budget") {
                            ...
                            this.TransferForm = response[response.length - 1].transferForm;
                            this.PoolBudgetForm = response[response.length - 1].poolBudgetForm;
                            this.strategy = "MillionBaht";
                            this.CostCenterOfVP = response[response.length - 1].costCenterOfVP;
                            this.CodeCostCenterOfVP = response[response.length - 1].codeCostCenterOfVP;
                            this.AvailableBudget = response[response.length - 1].availableBudget;
                            this.PoolID = response[response.length - 1].poolId;
                            this.PoolYear = this.year_now;
                                ...
                                this.capexService.GetCapexsInfo(poolID_, 'Requestpool').subscribe(response2 => {
                                    if (response2 != null && response2 != undefined) {
                                        ...
                                        if ((this.initiativeService.suggestionStatus.flowType === 'initiative') ? this.initiativeService.suggestionStatus.status === 'revised' : (this.initiativeService.suggestionStatus.subStage !== null && this.initiativeService.suggestionStatus.subStage !== undefined) ? this.initiativeService.suggestionStatus.subStatus === 'revised' : false) {
                                            this.AvailableBudget += (response[response.length - 1].capexType == 'AddmoreCapex') ? parseFloat(this.capexInformationForm.get('additionalCost').value) : parseFloat(this.capexInformationForm.get('projectCost').value);
                                        }
                                        // }
                                        // else {
                                        //   this.AvailableBudget = response[response.length - 1].availableBudget;
                            ...
                            this.additionalCost = response[response.length - 1].additionalCost;



                            this.capexInformationForm.patchValue({
                                capexInformationId: response[response.length - 1].capexInformationId,
                                projectCost: response[response.length - 1].projectCost,
                                startingDate_tmp: response[response.length - 1].startingDate,
                                projecctComRun_tmp: response[response.length - 1].projecctComRun,
                                requestIniNoDate_tmp: response[response.length - 1].requestIniNoDate,
                                showRemark_tmp: false,

                                existingCost: response[response.length - 1].existingBudget,
                                spendingActual: response[response.length - 1].spendingActual,

                                budgetAvailable: response[response.length - 1].existingBudget - response[response.length - 1].spendingActual,
                                additionalCost: response[response.length - 1].additionalCost,
                                totalAvailable: this.ProjectCost - response[response.length - 1].spendingActual,

                                //add
                                submitTo: response[response.length - 1].submitTo,
                                revistion: response[response.length - 1].revistion,
                                capexType: response[response.length - 1].capexType,
                                budgetYear: response[response.length - 1].budgetYear,
                                capexStatus: response[response.length - 1].capexStatus,
                                isMaxApprovedRev: response[response.length - 1].isMaxApprovedRev,
                                sequent: response[response.length - 1].sequent,
                                existingBudget: response[response.length - 1].existingBudget,
                                returnCost: response[response.length - 1].returnCost,
                                availableBudget: response[response.length - 1].availableBudget,
                                // availableBudgetPool: response[response.length - 1].availableBudgetPool,
                                actionYear: response[response.length - 1].actionYear,
                                poolYear: this.PoolYear,
                                costCenterOfVP: response[response.length - 1].costCenterOfVP,
                                codeCostCenterOfVP: response[response.length - 1].codeCostCenterOfVP
                            })
    ...
    changeRate(value, i) {
        const control = this.formGroup.get('capexInformationForm').get('AnnualForm').get('annualForm_list') as FormArray;
        ...
        if (control.at(i).get('currencyTitle').value != "" && control.at(i).get('currencyTitle').value != undefined &&
            ...
            const control1 = this.formGroup.get('capexInformationForm').get('monthForm0')?.get('monthForm_list') as FormArray;
            ...
            const control2 = this.formGroup.get('capexInformationForm').get('monthForm1')?.get('monthForm_list') as FormArray;
            if (control2) {
            ...
            const control3 = this.formGroup.get('capexInformationForm').get('monthForm2')?.get('monthForm_list') as FormArray;
            if (control3) {
            ...
            const control4 = this.formGroup.get('capexInformationForm').get('monthForm3')?.get('monthForm_list') as FormArray;
            if (control4) {
            ...
            const control5 = this.formGroup.get('capexInformationForm').get('monthForm4')?.get('monthForm_list') as FormArray;
            if (control5) {
            ...
            const control6 = this.formGroup.get('capexInformationForm').get('monthForm5')?.get('monthForm_list') as FormArray;
            if (control6) {
            ...
            const control7 = this.formGroup.get('capexInformationForm').get('monthForm6')?.get('monthForm_list') as FormArray;
            if (control7) {
            ...
            const control8 = this.formGroup.get('capexInformationForm').get('monthForm7')?.get('monthForm_list') as FormArray;
            if (control8) {
            ...
            const control9 = this.formGroup.get('capexInformationForm').get('monthForm8')?.get('monthForm_list') as FormArray;
            if (control9) {
            ...
            const control10 = this.formGroup.get('capexInformationForm').get('monthForm9')?.get('monthForm_list') as FormArray;
            if (control10) {
    ...
    ChangingValue_fx(ii) {
        const control = this.formGroup.get('capexInformationForm').get('AnnualForm').get('annualForm_list') as FormArray;
        const AnnualTotal1 = this.formGroup.get('capexInformationForm').get('AnnualForm').get('AnnualTotal1') as FormArray;
        let xx = +control.length
        let AnnualTotal_overall = 0;
        for (let i = 0; i < xx; i++) {
            let overall = 0;
    ...
    setRequestDate(date1) {
        if (date1 != null && date1 != '') {
            ...
            if (!isNaN(convertDate1.getTime()) && convertDate1.getTime() > 0) {
                ...
                if (this.progressService.capexLoading && (!this.initiativeService.isAddmore && !this.initiativeService.isRevise && !this.initiativeService.isReturn)) {
    ...
    clearMonthlyInvestmentTable() {
        this.year_m = [];
        // this.formGroup.get('capexInformationForm').get('requestIniNoDate').setValue(null);
        this.RequestIniNoDate = null;
        let capexForm = this.formGroup.get('capexInformationForm') as FormGroup;
    ...
    adjustAnnualAndMonthlyTable(dateStart: Date, type: string) {
        ...
        if ((d3 < d2) == false && type === "RequestIniNoDate" && (this.initiativeService.page != 'pool-create' && this.initiativeService.page != 'pool-edit')) {
            ...
            //auto fill data
            if (this.progressService.capexLoading && this.formGroup.get('progressForm')?.get('standardProjectDef')?.value && (!this.initiativeService.isAddmore && !this.initiativeService.isRevise && !this.initiativeService.isReturn)) {
    ...
    DisableCapexForm() {
        if (this.initiativeService.DisableTabCapex && !this.initiativeService.isReturn && !this.initiativeService.isAddmore) {
            if (this.formGroup.get('capexInformationForm'))
                this.capexInformationForm.disable();
            if (this.AnnualForm)
            ...
            // this.monthForm8.disable();
            // this.monthForm9.disable();
            this.capexTabStatusIsDisable = true;
        }
    }
    ...
    get getSettingAnual() {
        ...
        const start = this.dateUtil.convertDateUTC(settingData.startPeriodAnnual).getTime();
        const finish = this.dateUtil.convertDateUTC(settingData.finishPeriodAnnual).getTime();
        return (start <= today && today <= finish) && settingData.isAvailablePeriodAnnual ? false : true;
    }
    get getSettingMid() {
        const today = (this.dateUtil.GetToday).getTime();
        const settingData: SettingModel = this.settingService.settingData.value;
        ...
        const start = this.dateUtil.convertDateUTC(settingData.startPeriodMid).getTime();
        const finish = this.dateUtil.convertDateUTC(settingData.finishPeriodMid).getTime();
        return (start <= today && today <= finish) && settingData.isAvailablePeriodMid ? false : true;
    }

    // To compare between CostEstimation and ProjectCost.
    compareCostEstAndProjCost = (): void => {
        //RemarkShow
        // console.log(`DEB: ...compareCostEstAndProjCost`);
        const prjCost = +this.capexInformationForm.get('projectCost')?.value;
        // const costEst = this.generalData.costEstCapex;
        const costEst = this.costEstFromGeneral;
        // const f = this.initiativeService.getFormGroup

        console.log(`DEB: ProjectCost form = ${prjCost} is type ${typeof (prjCost)}`);
        console.log(`DEB: costEstCapex = ${costEst} is type ${typeof (costEst)}`);
        console.log(`DEB: childMessage = ${this.costEstFromGeneral} is type ${typeof (this.costEstFromGeneral)}`);
        console.log(`RemarkShow = ${this.RemarkShow} || IsAdmore = ${this.IsAddmore} || IsReturn = ${this.IsReturn}`);
        // console.log(`DEB: global form = ${f}`);
        if (prjCost != costEst) {
            console.log(`..Hitt ${prjCost} == ${costEst} , Not Equa`);
            this.RemarkShow = true;
        } else {
            console.log(`...Missed ${prjCost} == ${costEst} , Equals`);
            this.RemarkShow = false;
        }
    }

    getCurrentSpendingActual() {
        this.initiativeService.getOptionSpendingActual.subscribe(data => {
            const spendingActual = (parseFloat(data) / 1000000).toFixed(2)
            console.log('spendingActual: ', spendingActual);
            if (data) {
                this.capexInformationForm.get('spendingActual').setValue(spendingActual);
            }
        })
    }
  currentYearNumber() {
